...
ggplot(sims_estados %>% filter(prop_muestra==0.10) %>% 
         rename(theta = prop_obs, prop_observada = prop_casillas_muestra),
       aes(x = prop_observada, y = abs(sesgo), 
            colour = theta)) +
...
El patrón que observamos en estas gráficas se puede describir como

$${\textrm sesgo}\propto \theta(1-\theta)(1-prop_{obs})$$

```{r, echo =FALSE}
reg_tbl <- sims_estados %>% filter(prop_muestra==0.10) %>% 
  mutate(sesgo_abs = abs(sesgo)) %>% 
  rename(theta = prop_obs, prop_observada = prop_casillas_muestra) %>% 
  mutate(factor_f = theta*(1-theta)*(1-prop_observada))
...
ggplot(reg_tbl, aes(x = prop_observada,  
            colour = theta)) +
  geom_jitter(aes(y = abs(sesgo)), alpha = 0.5) +
  geom_line(aes(y = pred, group = theta)) +
  ylab("Sesgo absoluto") +
  xlab("Proporción de muestra recibida") +
  facet_wrap(~ state_abbr)
```
