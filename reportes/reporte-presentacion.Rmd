...
depedendiendo de caractarísticas de casillas, secciones y entidades correspondientes.
...
- Porcentaje de votos obtenidos para AMLO, JAMK, JHRC y RAC (INE, Cómputos distritales 2018)
...
ggplot(datos_check %>% filter(id!=71), aes(x = time, y = surv, group = id)) +
  ...
  facet_wrap(~ strata) + 
  ylab("% Censurado") + xlab("Horas después de las 18:30 (ajustada por huso)")
```

Abajo consideramos los efectos marginales de algunas variables (gráficas de dependencia
parcial, ver @friedman): 
...
Los coeficienta para el logaritmo de la proporción de votos recibidos por candidato son (valores
