...
llegadas_tbl_2 <- llegadas_tbl %>% filter(state_abbr %in% estados) %>%
                  ...
                  mutate(total_3 = RAC_1 + AMLO_1 + JAMK_1)
reg_2 <- survreg(Surv(tiempo_huso, status) ~ 1 +
                   ln_log_c +
                   tipo_casilla +
                   #tipo_seccion +
                   #state_abbr:log(1+RAC_1/total_3) + 
                   state_abbr:log(1+AMLO_1/(total_3 + 1)) + 
                   state_abbr:log(1+JAMK_1/(total_3 +  1)) +
                   state_abbr:ln_log_c + 
...
seleccionar_muestra <- function(conteo, prop = 0.07, est = "CHIH"){
  conteo_tbl <- conteo %>% 
    ...
    mutate(tipo_casilla = factor(TIPO_CASILLA, levels= c("B", "C", "E", "S")),
           lista_nominal_log = log(1 + LISTA_NOMINAL_CASILLA)) %>%
    mutate(ln_log_c = lista_nominal_log - media_ln_log) %>% 
    mutate(total_3 = RAC_1 + AMLO_1 + JAMK_1) %>% 
    select(state_abbr, tipo_casilla, ln_log_c, total_3, RAC_1, AMLO_1, JAMK_1) %>% 
    mutate(huso = case_when(state_abbr %in% c("BC", "SON") ~ 2,
                          state_abbr %in% c("CHIH", "BCS", "NAY", "SIN") ~ 1,
                          TRUE ~ 0)) 
  conteo_tbl
...
graficar_acumulado <- function(muestra_tiempo){
  datos <- muestra_tiempo %>% arrange(tiempo) %>% 
    mutate(acumulado_amlo = cumsum(AMLO_1), acumulado_tot = cumsum(RAC_1 + AMLO_1 + JAMK_1)) %>% 
...
g_1 <- graficar_acumulado(muestra_tiempo %>% filter(status == 1)) +
  ylim(c(0.25,0.75)) 
g_2 <- graficar_acumulado(muestra_tiempo %>% filter(status == 1) %>% 
                            mutate(tiempo = sample(tiempo, length(tiempo)))) +
  ylim(c(0.25, 0.75)) 
g_1 + g_2
```
...
simular_cortes <- function(cortes = cortes, prop_muestra = 0.3) {
  evaluacion_tbl <- map(cortes, function(corte) {  
    ...
    prop_corte <- datos %>% filter(tiempo <= corte) %>% pull(prop_amlo) %>% last
    hora_salida <- datos %>% filter(tiempo <= corte) %>% pull(tiempo) %>% last
    prop <- datos %>% pull(prop_amlo) %>% last
    tibble(prop_corte = prop_corte, prop = prop, hora_salida = hora_salida)
  }) %>% bind_rows %>% 
      mutate(corte = corte)
  }) %>% bind_rows
}
eval_tbl_1 <- simular_cortes(cortes, 0.05) %>% mutate(prop_muestra = 0.05)
eval_tbl_2 <- simular_cortes(cortes, 0.10) %>% mutate(prop_muestra = 0.10)
eval_tbl_3 <- simular_cortes(cortes, 0.20) %>% mutate(prop_muestra = 0.20)
...
prop_real <- sum(total$RAC_1, na.rm = T)/
  sum(total$RAC_1 + total$JAMK_1 + total$AMLO_1, na.rm = T)
prop_real
```
...
ecm_tbl
ggplot(ecm_tbl, aes(x = corte, y = recm^2, 
...
## Análisis según tamaño de muestra 

```{r}
ecm_tbl <- evals_tbl %>% mutate(prop_v = prop_real) %>% 
  group_by(corte, prop_muestra) %>% 
  summarise(sesgo_2 = mean((prop_corte - prop_real)^2),
            varianza = var(prop_corte), hora_salida_prom = mean(hora_salida)) %>% 
  mutate(recm = sqrt(sesgo_2 + varianza)) 
ecm_tbl
ggplot(ecm_tbl, aes(x = corte, y = recm, label = round(hora_salida_prom,2), 
                    colour = factor(prop_muestra), 
                    group=prop_muestra)) +
  geom_point() + geom_line() + geom_text(nudge_y = 0.001)
```


```{r}
ggplot(ecm_tbl, aes(x = corte, y = sesgo_2 / recm^2, label = round(hora_salida_prom,2), 
                    colour = factor(prop_muestra), 
                    group=prop_muestra)) +
  geom_point() + geom_line() + geom_text(nudge_y = 0.001)
```
