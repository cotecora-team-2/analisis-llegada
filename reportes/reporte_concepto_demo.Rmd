...
reg_2 <- survreg(Surv(tiempo_huso, status) ~ 1 +
                   ...
                   strata(state_abbr), 
                   llegadas_tbl_2, 
                 dist='loglogistic', control = survreg.control(maxiter = 5000))
```
...
```{r, echo = FALSE}
simular_cortes <-  function(rep, cortes = cortes, prop_muestra = 0.3){
...
sim_1 <- simular_cortes("a", cortes = cortes, prop_muestra = 0.05)
sim_1
```
```{r}
eval_tbl_1 <- map_df(1:400, ~ simular_cortes(.x, cortes= cortes, prop_muestra = 0.05))
eval_tbl_2 <- map_df(1:400, ~ simular_cortes(.x, cortes= cortes, prop_muestra = 0.10))
eval_tbl_3 <- map_df(1:400, ~ simular_cortes(.x, cortes= cortes, prop_muestra = 0.20))
```
...
ecm_tbl <- evals_tbl %>% 
  ...
  summarise(sesgo_2 = (mean(prop_cand) - prop_obs)^2,
            varianza = var(prop_cand), .groups = "drop") %>% 
  ungroup %>% 
...
```{r, echo = FALSE, fig.height = 4, fig.width = 10}
ggplot(ecm_tbl, aes(x = corte, y = sqrt(sesgo_2) / sqrt(varianza), 
                    colour = prop_muestra, 
                    group=prop_muestra)) +
